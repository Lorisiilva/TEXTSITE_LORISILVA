...
function createCheckboxWithLabel(parent, label, key, value) {
  ...
  checkbox.changed(() => {
    ...
    if (key === "randomAppearance") {
      updateParticleColors(); // Ensure particles update instantly
    }
  });
  container.child(labelEl);
...
function updateParticleColors() {
  for (let point of textPoints) {
    ...
    point.color = params.randomAppearance
      ? color(random(255), random(255), random(255))
      : color(params.particleColor);
    point.size = params.randomAppearance ? random(1, 10) : params.particleSize;

    // Add random velocity for movement
    if (params.randomAppearance) {
      point.vel = p5.Vector.random2D().mult(random(2, 5)); // Assign random movement direction and speed
    } else {
      // Reset velocity for static behavior
      point.vel.set(0, 0);
    }
  }
}
